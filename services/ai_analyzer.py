            ...
            async with httpx.AsyncClient(timeout=self.API_TIMEOUT) as client:
                ...
                if stream:
                    ...
                    async with client.stream("POST", api_url, json=request_data, headers=headers) as response:
                        ...
                        async def process_sse_line(line: str, *, log_decode_error_as_debug: bool = False):
                            ...
                            try:
                                ...
                                if log_decode_error_as_debug:
                                    logger.debug(f"行缓冲区剩余数据无法解析: {line[:200]}")
                                else:
                            ...
                            if error_data is not None:
                                logger.error(f"流式响应中收到错误: {error_data}")
                                return json.dumps({
                                    "stock_code": stock_code,
                                    "error": f"流式响应错误: {error_data}",
                                    "status": "error"
                                })
                        ...
                        async for chunk in response.aiter_text():
                            ...
                            # 按换行分割，最后一段可能不完整，保留在buffer中
                            parts = line_buffer.split('\n')
                            line_buffer = parts[-1]
                            lines = parts[:-1]

                            for line in lines:
                                processed_chunk = await process_sse_line(line)
                                if processed_chunk:
                        ...
                        if line_buffer.strip():
                            processed_chunk = await process_sse_line(
                                line_buffer,
                                log_decode_error_as_debug=True
                            )
                            if processed_chunk:
                                yield processed_chunk
                                processed_data = json.loads(processed_chunk)
